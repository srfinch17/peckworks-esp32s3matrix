...
// headroom, and the baked-frames library (data/frames/*.cfr + index.json)
// must be internally consistent. Reuses build-release.mjs's exact mechanism
// for locating mklittlefs and parsing the SPIFFS partition size.
//
...
async function main() {
  ...
  // (c) frames/*.cfr <-> index.json parity, both directions.
  const index = JSON.parse(await readFile(path.join(FRAMES_DIR, "index.json"), "utf8"));
  const diskFiles = new Set(readdirSync(FRAMES_DIR).filter((f) => f.endsWith(".cfr")));
  const indexFiles = new Set(index.animations.map((a) => a.file));
  const orphans = [...diskFiles].filter((f) => !indexFiles.has(f));
  const missing = [...indexFiles].filter((f) => !diskFiles.has(f));
  assert(orphans.length === 0 && missing.length === 0,
    `frames/*.cfr matches index.json 1:1 (${diskFiles.size} files, ${indexFiles.size} indexed` +
    (orphans.length ? `, orphans on disk: ${orphans.join(", ")}` : "") +
    (missing.length ? `, missing from disk: ${missing.join(", ")}` : "") + ")");

  // (d) each entry's shape: frame count and name pattern.
